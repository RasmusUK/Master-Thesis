...
public class SnapshotPerformanceTests : MongoIntegrationTestBase
{
    ...
    public async Task ReplaySnapshotPerformance(int eventCountSnapshot)
    {
        ...
        for (var i = 1; i <= eventCount; i++)
        {
            if (i % 10000 == 0 || i % 25000 == 0)
                testOutputHelper.WriteLine(
                    $"Creating entity {i} of {eventCount} with max duration {durations.OrderByDescending(x => x).Take(1).First()} ms");
        ...
        for (var i = 0; i < eventThresholds.Length; i++)
        {
            ...
            testOutputHelper.WriteLine(
                $"Replay without snapshot for {eventThresholds[i]} events took: {sw.ElapsedMilliseconds} ms");
        }
        ...
        for (var i = 0; i < eventThresholds.Length; i++)
        {
            ...
            testOutputHelper.WriteLine(
                $"Replay with snapshot for {eventThresholds[i]} events took: {sw.ElapsedMilliseconds} ms");
        }
    }
}